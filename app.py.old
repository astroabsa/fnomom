...
def style_df(df: pd.DataFrame, bullish: bool):
    if df.empty:
        return df
    ref_col = '15m High' if bullish else '15m Low'
    fmt = {
    ...
    def color_chg(val):
        ...
        return ''
    def color_badge(val):
        return 'color:#16a34a;font-weight:700' if val == '✅'                else 'color:#94a3b8'
    return (df.style
...
with left:
    ...
    if bull_df.empty:
        ...
        st.dataframe(style_df(bull_df, True),
...
with right:
    ...
    if bear_df.empty:
        ...
        st.dataframe(style_df(bear_df, False),
                     use_container_width=True, hide_index=True, height=460)
